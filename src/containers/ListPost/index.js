...
import { Container, Icon, Toast, Thumbnail } from "native-base";
/* Services */
...
class ListPost extends React.PureComponent {
  ...
  async componentDidMount() {
    ...
    this.backHandler = BackHandler.addEventListener("hardwareBackPress", () =>
      this.goBack(this.state.isFocused)
    );
  ...
  getProfile = async () => {
    if (this.props.isAuthenticate) {
      ...
      user_profile(profilePayload, headers)
        .then(res => {
  ...
  loadPosts = () => {
    if (this.props.isAuthenticate) {
      ...
      if (
        ...
        try {
          news_feed(payload, headers)
            .then(response => {
              /* take payload backup to check for changes later */
              if (
                this.payloadBackup.length === response.data.data.posts.length
              ) {
                ...
                response.data.data.posts.map((item, index) => {
                  item.Item.likeCount =
                    response.data.data.counts[index].likeCount;
                  item.Item.commentCount =
                    response.data.data.counts[index].commentCount;
                  item.Item.addOnPoints =
                    response.data.data.counts[index].addOnPoints;
                });
                if (
                  JSON.stringify(this.posts) !==
                  JSON.stringify(response.data.data.posts)
                ) {
                ...
                if (this.postList.length !== 0) {
                  if (
                    this.posts.length > this.payloadBackup.length &&
  ...
  deletePost = async postId => {
    if (this.props.isConnected) {
      ...
      try {
        await delete_post(payload, headers)
          .then(res => {
            if (res.status === 200) {
              var index = this.posts.findIndex(post => {
                return post.Item.post_id == postId;
              });
  ...
  gotoFeedbackPageAlert = () => {
    if (this.props.isAuthenticate) {
      if (
        this.props.feedbackCurrentCount !== 0 &&
        this.props.feedbackCurrentCount % feedbackDisplayCount == 0
      ) {
        Alert.alert(
          "Loving HappyWorks?",
  ...
  createTiles = async posts => {
    ...
    await posts.map(async item => {
      ...
      if (posts.length == this.postList.length) {
        setTimeout(
          () => this.setState({ refreshing: false, postList: this.postList }),
          1500
        );
      }
    });
  ...
  render() {
    return (
      <Container style={{ backgroundColor: "#eee" }}>
        ...
        >
          {!this.state.initialLoad ? (
            ...
            <View style={{ alignItems: "center", justifyContent: "center" }}>
              <Text style={{ margin: 10 }} key={0}>
                No post to display
              </Text>
              <Text style={{ margin: 10 }} key={1}>
                Create a new post by clicking on + icon
              </Text>
            </View>
          )}
        </ScrollView>
        ...
        <LoadingModal enabled={this.state.isPostDeleted} />
      </Container>
    );
  }
}
...
const mapDispatchToProps = dispatch => {
  return {
    updateWallet: props =>
      dispatch({ type: dev.UPDATE_WALLET, payload: props }),
    imageUrl: props => dispatch({ type: user.UPDATE_IMAGE, payload: props }),
    deAuthenticate: () => dispatch({ type: auth.DEAUTHENTICATE_USER }),
    updateNewTokens: props =>
      dispatch({ type: auth.REFRESH_TOKEN, payload: props }),
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withInAppNotification(ListPost));
